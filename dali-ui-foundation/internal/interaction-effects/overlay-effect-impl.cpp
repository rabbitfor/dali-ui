...
void OverlayEffectImpl::OnAttached(TraitId /*id*/, View& view)
{
  ...
  AttachToInteractiveView(view);
}

void OverlayEffectImpl::AttachToInteractiveView(View view)
{
  OverlayEffectDataTrait data = GetOrCreateOverlayEffectDataTrait(view);
  data.GetImpl().AttachEffect(this);
  data.GetImpl().SuppressDefaultFocusIndicator(view);
...
: TraitObject(),
  ...
  mActiveOverlay(),
  mActiveTarget(),
  mDefaultFocusIndicatorOwner(),
  mPressed(false),
  mFocused(false),
  mDefaultFocusIndicatorEnabledBeforeAttach(true),
  mDefaultFocusIndicatorSuppressed(false)
{
}
...
void OverlayEffectDataTraitImpl::SetActiveTarget(View target)
{
  mActiveTarget = target;
}

View OverlayEffectDataTraitImpl::GetActiveTarget() const
{
  return mActiveTarget;
}

void OverlayEffectDataTraitImpl::ClearActiveTarget()
{
  mActiveTarget.Reset();
}
...
void OverlayEffectDataTraitImpl::SuppressDefaultFocusIndicator(View owner)
{
  if(!owner)
  {
    return;
  ...
  mDefaultFocusIndicatorOwner               = owner;
  mDefaultFocusIndicatorEnabledBeforeAttach = owner.IsDefaultFocusIndicatorEnabled();
  if(mDefaultFocusIndicatorEnabledBeforeAttach)
  {
    owner.SetDefaultFocusIndicatorEnabled(false);
    mDefaultFocusIndicatorSuppressed = true;
  }
}

void OverlayEffectDataTraitImpl::RestoreDefaultFocusIndicator()
{
  if(!mDefaultFocusIndicatorSuppressed)
  {
    mDefaultFocusIndicatorOwner = View();
    return;
  }

  if(mDefaultFocusIndicatorOwner)
  {
    mDefaultFocusIndicatorOwner.SetDefaultFocusIndicatorEnabled(mDefaultFocusIndicatorEnabledBeforeAttach);
  }
