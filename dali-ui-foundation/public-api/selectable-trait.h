 ...
 * by leveraging the owner View's ClickableTrait.
 *
 * Unlike ClickableTrait, SelectableTrait does not handle key events or focus
 * directly. It uses a separate reserved trait slot (SELECTABLE_TRAIT) and can
 * coexist with ClickableTrait on the same View.
 *
 * @note When toggle-by-click is enabled and the owner View does not have a
 * ClickableTrait, one is automatically created and attached.
 */
...
public: // Signals
   ...
   */
  Signal<void(View, bool)>& SelectionChangedSignal();
...
public: // API
   ...
   * already have a ClickableTrait, one is automatically created and attached.
   *
   * @param[in] enabled True to enable toggle-by-click, false to disable (default: true)
   */
