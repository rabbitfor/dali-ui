...
> [!IMPORTANT]
> `SetTouchFocusable(true)`는 해당 View를 터치했을 때 그 View 자체가 포커스를 받아야 하는 경우에만 사용하세요. 다른 focus target을 조작하기 위한 command button은 일반적으로 touch-focusable로 만들지 않는 편이 좋습니다. 그렇지 않으면 조작 대상의 focus를 버튼이 다시 가져갈 수 있습니다.
...
장식용 subtree, 그룹 단위로 disabled된 영역, 또는 focusable View가 없다는 것이 명확한 subtree에 사용하세요. 해당 ancestor 아래의 descendant는 `RequestFocus()`, `SetCurrentFocusView()`, focus navigation에서 거부됩니다.
...
// child delegation을 포함한 focus 요청
...
일반적인 application focus 요청에는 `RequestFocus()`를 사용하세요. Android와 유사하게 focus request를 resolve합니다:

1. 대상 View에 focus request를 보냅니다.
2. container는 eligible focusable descendant로 요청을 위임할 수 있습니다.
3. 최종 resolve된 View가 current focus로 설정됩니다.
4. 대상 또는 candidate가 blocked 상태이거나 scene에 없거나 disabled, invisible, non-focusable이면 실패합니다.
...
FocusGroup은 containment boundary입니다. focus가 FocusGroup 내부에 있을 때 기본 keyboard focus 이동은 해당 subtree 안으로 제한됩니다. 패널, 다이얼로그, 팝업, 컴포넌트 내부처럼 arrow key나 Tab / Shift+Tab으로 focus가 밖으로 빠져나가면 안 되는 영역에 사용하세요.
